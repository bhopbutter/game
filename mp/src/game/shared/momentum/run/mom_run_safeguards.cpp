...
#ifdef CLIENT_DLL
        return IsMovementKeysSafeguarded(::input->GetButtonBits(engine->IsPlayingDemo()));
#elif GAME_DLL
        return IsMovementKeysSafeguarded(pPlayer->m_nButtons);
#endif
...
{
    if (gpGlobals->curtime > m_flLastTimePressed + MOM_DOUBLEPRESS_MAXTIME)
    {
...
{
    m_arrpSafeguards[RUN_SAFEGUARD_PRACTICEMODE] = new CRunSafeguard("enable practice mode");
    m_arrpSafeguards[RUN_SAFEGUARD_RESTART] = new CRunSafeguard("restart map");
    m_arrpSafeguards[RUN_SAFEGUARD_SAVELOC_TELE] = new CRunSafeguard("teleport to saveloc");
    m_arrpSafeguards[RUN_SAFEGUARD_CHAT_OPEN] = new CRunSafeguard("open chat");
}
...
{
    return m_arrpSafeguards[type]->IsSafeguarded(mode);
}
